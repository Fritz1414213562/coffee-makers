...
//	// constructor for a half-fixed matrix
//	Matrix(const int& runtime_length) {
//		static_assert(
//		(Row_CompileTime > 0) && (Col_CompileTime < 0) ||
//		(Row_CompileTime < 0) && (Col_CompileTime > 0),
//		"In the case that 'makers::Matrix' is fixed, the constructor (int const&) is forbidden.");
//
//		if constexpr (Row_CompileTime < 0) {
//			_row = runtime_length;
//			_col = Col_CompileTime;
//		}
//		else if constexpr (Col_CompileTime < 0) {
//			_row = Row_CompileTime;
//			_col = runtime_length;
//		}
//		_size = _row * _col;
//		_matrix.resize(_size, 0);
//	}
	...
	template<typename Integer, typename std::enable_if<
		value_conjunction<
			...
			value_disjunction<(Row < 0), (Col < 0)>::value>::value,
	...
	template<typename Integer, typename std::enable_if<
		value_conjunction<
			std::is_same<Integer, int>::value,
			value_conjunction<(Row > 0), (Col > 0)>::value>::value,
		std::nullptr_t>::type = nullptr>
	Matrix(const Integer& runtime_row, const Integer& runtime_col) :
		_row(runtime_row),
		_col(runtime_col),
